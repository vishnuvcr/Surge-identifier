...
from scripts.run_cpr_multimode import (
    args, load_frames, valid_geometry, trade_row, intraday_trades,
    swing_trades, btst_trades, metrics,
)
...
def main():
    ...
    for mode in modes:
        ...
        for strategy in strategies:
            if mode == 'intraday':
                sig = generate_intraday(x, strategy, cfg)
                ...
                sig['strategy_id'] = strategy
                if mode == 'swing':
                    td, invalid = swing_trades(sig, a.capital, a.risk_fraction, a.max_position_fraction, roundtrip, len(symbols), a.max_swing_days)
                else:
                    td, invalid = btst_trades(sig, raw, roundtrip, a.risk_fraction, a.max_position_fraction, len(symbols), a.capital)
            if not td.empty:
                td.to_csv(out / f'{mode}__{strategy}__trades.csv', index=False)
            m = metrics(td, a.capital)
            m['invalid_geometry_signals'] = invalid
            m['strategy_id'] = strategy
            summary['modes'][mode][strategy] = m
