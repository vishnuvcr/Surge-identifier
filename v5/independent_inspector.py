...
def main() -> int:
    ...
    # Cheap independent inspector must gate the expensive backtest.
    ...
    # Production must never bypass the guarded runner.
    ...
    # Learned failure: Date/date schema drift.
    ...
    # Learned failure: isolated payoff anomalies must be contained, not swallowed wholesale.
    ...
    # Guardrails inherited from prior runs.
    ...
    # AST sanity catches malformed Python before any data download.
    ...
    report = {'blocking_failures': failures, 'warnings': warnings, 'memory_rules': len(lessons), 'policy': 'Never spend hours downloading/backtesting when a cheap structural check can detect the problem first.'}
