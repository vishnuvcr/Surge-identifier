...
def _symbols(weights: pd.DataFrame) -> list[str]:
    cols = [c for c in weights.columns if c != 'DATE']
...
def download_prices(weights: pd.DataFrame) -> dict:
    symbols = _symbols(weights); failures=[]; workers=int(os.environ.get('V62_PRICE_WORKERS','8'))
...
def download_flows() -> dict:
    out=CACHE/'v62_fii_dii_raw.xlsx'; url=CFG['flows']['fallback_url']
    if not out.exists():
        r=requests.get(url,headers=UA,timeout=120); r.raise_for_status(); out.write_bytes(r.content)
    parsed=[]; xl=pd.ExcelFile(out)
    for sheet in xl.sheet_names:
        try: raw=pd.read_excel(out,sheet_name=sheet)
        except Exception: continue
        if raw.empty: continue
        raw.columns=[str(c).strip().lower() for c in raw.columns]; date_col=next((c for c in raw.columns if 'date' in c),None)
        if not date_col: continue
        raw[date_col]=pd.to_datetime(raw[date_col],errors='coerce',dayfirst=True).dt.normalize()
        for side,needles in {'fii_net':['fii net','fii/fpi net','fii net value','fpi net'],'dii_net':['dii net','dii net value']}.items():
            col=next((c for c in raw.columns if any(n in c for n in needles)),None)
            if col:
                z=raw[[date_col,col]].rename(columns={date_col:'date',col:side}); z[side]=pd.to_numeric(z[side].astype(str).str.replace(',','',regex=False),errors='coerce'); parsed.append(z)
    if parsed:
        df=parsed[0]
