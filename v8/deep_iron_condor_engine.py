...
def load_inputs():
    ...
    for c in ['open','high','low','close','volume']:
        if c in index.columns: index[c] = pd.to_numeric(index[c], errors='coerce')
    ...
    options = options.dropna(subset=['date','expiry','strike','close']); options = options[options['close'] > 0].copy()
    futures = futures.dropna(subset=['date','expiry']).copy()
    for c in ['close','open_interest','volume']:
        if c in futures.columns: futures[c] = pd.to_numeric(futures[c], errors='coerce')
    ...
    prices.columns = [str(c).strip().lower() for c in prices.columns]; prices['symbol'] = prices['symbol'].astype(str).str.upper()
    prices['date'] = pd.to_datetime(prices['date'], errors='coerce').dt.normalize(); prices['close'] = pd.to_numeric(prices['close'], errors='coerce')
    prices = prices.dropna(subset=['symbol','date','close']).drop_duplicates(['symbol','date'], keep='last')
    weights.columns = [str(c).strip().upper() for c in weights.columns]; weights['DATE'] = pd.to_datetime(weights['DATE'], errors='coerce').dt.normalize()
    for c in weights.columns:
        if c != 'DATE': weights[c] = pd.to_numeric(weights[c], errors='coerce').fillna(0.0)
    membership.columns = [str(c).strip().lower() for c in membership.columns]; membership['symbol'] = membership['symbol'].astype(str).str.upper()
    membership['valid_from'] = pd.to_datetime(membership['valid_from'], errors='coerce').dt.normalize(); membership['valid_to'] = pd.to_datetime(membership['valid_to'], errors='coerce').dt.normalize()
    flows['date'] = pd.to_datetime(flows['date'], errors='coerce').dt.normalize()
    for c in ['fii_net','dii_net']:
        if c in flows.columns: flows[c] = pd.to_numeric(flows[c], errors='coerce')
...
def make_constituent_features(dates, prices, weights, membership):
    p = prices.pivot_table(index='date', columns='symbol', values='close', aggfunc='last').sort_index(); r = p.pct_change(fill_method=None)
    symbols = [c for c in weights.columns if c != 'DATE' and c in r.columns]
    if not symbols: return pd.DataFrame(index=pd.DatetimeIndex(dates))
    w = pd.merge_asof(pd.DataFrame({'date':pd.DatetimeIndex(dates)}), weights[['DATE']+symbols], left_on='date', right_on='DATE', direction='backward', allow_exact_matches=False).set_index('date')
    W = w[symbols].fillna(0.0); R = r.reindex(pd.DatetimeIndex(dates))[symbols]
    for sym, g in membership.groupby('symbol'):
        ...
        W.loc[~valid, sym] = 0.0
    wsum = W.sum(axis=1).replace(0,np.nan); cnt = (W > 0).sum(axis=1).replace(0,np.nan)
    out = pd.DataFrame(index=W.index); out['const_wret1'] = (R.fillna(0.0)*W).sum(axis=1)/wsum; out['const_breadth'] = (((R > 0).astype(float)*(W > 0)).sum(axis=1)/cnt); out['const_dispersion'] = R.std(axis=1)
    return out.replace([np.inf,-np.inf],np.nan).fillna(0.0)


def make_global_features(index, futures, context, flows, news, prices, weights, membership):
    dates = pd.DatetimeIndex(index['date'].unique()).sort_values(); x = index.set_index('date').sort_index(); c = x['close'].astype(float); r = c.pct_change(fill_method=None)
    g = pd.DataFrame(index=dates); g['ret1']=r; g['ret5']=c.pct_change(5,fill_method=None); g['ret20']=c.pct_change(20,fill_method=None); g['ret60']=c.pct_change(60,fill_method=None); g['vol10']=r.rolling(10).std(); g['vol20']=r.rolling(20).std(); g['ema20_gap']=c/c.ewm(span=20,adjust=False).mean()-1
    up=r.clip(lower=0).ewm(alpha=1/14,adjust=False).mean(); dn=(-r.clip(upper=0)).ewm(alpha=1/14,adjust=False).mean(); g['rsi14']=up/(dn+1e-9)
    ...
    g=g.join(make_constituent_features(dates,prices,weights,membership))
    age_flow=int(CFG['freshness']['flow_max_age_days'])
    if {'fii_net','dii_net'}.issubset(flows.columns):
        ...
        for col in ff.columns: g[col]=a[col].where(age<=age_flow,0.0)
    ...
    if pos or neg or cnt:
        ...
        for col in nd.columns: g[col]=a[col].where(age<=age_news,0.0)
...
def make_option_features(index, options):
    ...
    for typ in ['CE','PE']:
        ...
        for stat in ['open_interest','volume']:
            h=z.groupby(['date','m'])[stat].sum().unstack(fill_value=0).reindex(dates).fillna(0.0); h=h.div(h.sum(axis=1).replace(0,1),axis=0)
            for b in range(-8,9): out[f'opt_{typ.lower()}_{stat[:2]}_{b:+d}']=h[b] if b in h.columns else 0.0
    ce=o[o['option_type'].eq('CE')].groupby('date')['open_interest'].sum(); pe=o[o['option_type'].eq('PE')].groupby('date')['open_interest'].sum(); out['opt_pcr_oi']=pe/ce.replace(0,np.nan)
    return out.replace([np.inf,-np.inf],np.nan).fillna(0.0)
...
def build_samples(inp):
    ...
    for i in range(seq-1,len(dates),int(CFG['sample_stride'])):
        ...
        td=pd.Timestamp(path_dates[-1]); w=idx.loc[path_dates]; spot=float(idx.loc[d,'close']); close_ret=float(idx.loc[td,'close'])/spot-1; hi=float(w['high'].max() if 'high' in w.columns else w['close'].max()); lo=float(w['low'].min() if 'low' in w.columns else w['close'].min()); path_hi=hi/spot-1; path_lo=lo/spot-1
...
class ICTransformer(nn.Module):
    ...
    def forward(self,x):
        h=self.norm(self.enc(self.inp(x))[:,-1]); return torch.sort(self.head(h).view(-1,3,7),dim=-1).values
...
def fit_model(X,meta):
    y=meta[['target_return','target_down_exc','target_up_exc']].to_numpy(np.float32); xm=X.reshape(-1,X.shape[-1]).mean(0); xs=X.reshape(-1,X.shape[-1]).std(0); xs=np.where(xs<1e-6,1,xs); Xn=np.clip((X-xm)/xs,-8,8).astype(np.float32); n=len(meta); cut=max(1,int(n*0.9)); tr=np.arange(cut); va=np.arange(cut,n) if cut<n else tr; model=ICTransformer(X.shape[-1]); opt=torch.optim.AdamW(model.parameters(),lr=float(CFG['model']['learning_rate']),weight_decay=float(CFG['model']['weight_decay'])); dl=DataLoader(TensorDataset(torch.from_numpy(Xn[tr]),torch.from_numpy(y[tr])),batch_size=int(CFG['model']['batch_size']),shuffle=False); best=1e99; state=None; bad=0
    for _ in range(int(CFG['model']['epochs'])):
        model.train()
        for xb,yb in dl:
            opt.zero_grad(set_to_none=True); q=model(xb); loss=pinball(q[:,0],yb[:,0])+pinball(q[:,1],yb[:,1])+pinball(q[:,2],yb[:,2]); loss.backward(); torch.nn.utils.clip_grad_norm_(model.parameters(),float(CFG['model']['gradient_clip'])); opt.step()
        model.eval()
        with torch.no_grad(): q=model(torch.from_numpy(Xn[va])); val=float(pinball(q[:,0],torch.from_numpy(y[va,0]))+pinball(q[:,1],torch.from_numpy(y[va,1]))+pinball(q[:,2],torch.from_numpy(y[va,2])))
...
def predict(model,norm,X):
    ...
    low=raw[:,0]-raw[:,1]; close=raw[:,0]; high=raw[:,0]+raw[:,2]; low=np.minimum(low,close); high=np.maximum(high,close); return np.stack([low,close,high],axis=1)


def sample_q(q): return np.interp(np.linspace(0.0025,0.9975,400),Q,q)
...
def splits(meta,kind):
    ...
    if kind=='wfo':
        ...
        for i,s in enumerate(starts):
            te=pre[s:s+nt]; test=meta[meta['expiry'].isin(te)].copy(); test_start=pd.Timestamp(test['signal_date'].min()); train=meta[(meta['expiry']<te[0]-pd.Timedelta(days=int(CFG['purge_days'])))&(meta['signal_date']<test_start-pd.Timedelta(days=int(CFG['embargo_days'])))].copy()
    ...
    if kind=='new_oos': test=meta[meta['signal_date']>=pd.Timestamp(CFG['new_oos_start'])].copy()
        ...
        split_exp=exps[int(len(exps)*0.7)]; test=meta[meta['expiry']>=split_exp].copy()
...
def backtest_case(case,X,meta,book):
    train,test,name=case; train_idx=train.index.to_numpy(); test_idx=test.index.to_numpy(); model,norm=fit_model(X[train_idx],train); pm=pred_metrics(model,norm,X[test_idx],test,train); tr,lad=run_strategy(model,norm,X[test_idx],test.reset_index(drop=True),book); tm=trade_metrics(tr); return {'name':name,'train_samples':len(train),'test_samples':len(test),**pm,**{f'strategy_{k}':v for k,v in tm.items()}},tr,lad
...
def main():
    ...
    pd.DataFrame(wfo_rows).to_csv(OUT/'wfo_metrics.csv',index=False); pd.concat(wfo_trades,ignore_index=True).to_csv(OUT/'wfo_strategy_trades.csv',index=False) if any(not x.empty for x in wfo_trades) else pd.DataFrame().to_csv(OUT/'wfo_strategy_trades.csv',index=False)
    ...
    if not alltr.empty:
        alltr[alltr['split'].eq('70_30')].to_csv(OUT/'split_70_30_trades.csv',index=False); alltr[alltr['split'].eq('new_oos')].to_csv(OUT/'new_oos_trades.csv',index=False)
    else:
        pd.DataFrame().to_csv(OUT/'split_70_30_trades.csv',index=False); pd.DataFrame().to_csv(OUT/'new_oos_trades.csv',index=False)
